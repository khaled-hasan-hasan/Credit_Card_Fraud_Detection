...
from credit_fraud_utils_helper import load_config ,  save_model_comparison
...
def train_lda_model(X_train_scaled, y_train, X_val_scaled, y_val, random_seed, model_comparison, trainer):
    best_params = {}

    if trainer['trainer']['lda']['grid_search'] == True:
        param_grid = {
            'solver': ['svd', 'lsqr', 'eigen'],
            'shrinkage': [None, 'auto']  # Only used if solver is 'lsqr' or 'eigen'
        }
        ...
        best_params = trainer['trainer']['LDA']['parameters']

    # Train LDA with best or default params
    lda = LDA(**best_params)
    ...
    return {"model": lda, "parameters": best_params, "threshold": optimal_threshold}
...
if __name__ == "__main__":
    ...
    if trainer['trainer']['Voting_Classifier']['train']:
        models['Voting_Classifier'] = (X_train, y_train, X_val, y_val, models, RANDOM_SEED, model_comparison, trainer)
    

    if trainer['trainer']['lda']['train']:
        models['lda'] = train_lda_model(X_train_scaled, y_train, X_val_scaled, y_val, RANDOM_SEED, model_comparison,  trainer)
