...
def evaluate_model(model, model_comparison, path, title, X_train, y_train, x_val, y_val, evaluation_config):
    ...
    if evaluation_config['train'] == True:
        ...
        eval_classification_report_confusion_matrix(y_train, y_train_pred, title + ' train', save_png=save_cm_plots,
                                                    path=eval_plots_path)
        eval_precision_recall_for_different_threshold(y_pred_prob=y_train_pred_proba, y_true=y_train,
                                                      title=title + ' train', save_png=save_pr_rc_th_plots,
                                                      path=eval_plots_path)
        eval_precision_recall_curve(y_pred_prob=y_train_pred_proba, y_true=y_train, title=title + ' train',
                                    save_png=save_roc_curve, path=eval_plots_path)

    if evaluation_config['validation'] == True:

        y_val_pred = model.predict(x_val)
        y_val_pred_proba = model.predict_proba(x_val)[:, 1]
        report_val = eval_classification_report_confusion_matrix(y_val, y_val_pred, title + ' validation',
                                                                 save_png=save_cm_plots, path=eval_plots_path)
        eval_precision_recall_curve(y_pred_prob=y_val_pred_proba, y_true=y_val, title=title + ' validation',
                                    save_png=save_roc_curve, path=eval_plots_path)
        ...
        if evaluation_config['metric']['PR_AUC'] == True:
            y_val_pred_proba = model.predict_proba(x_val)[:, 1]
            model_comparison[title]['PR AUC'] = eval_auc_precision_recall_curve(y_pred_prob=y_val_pred_proba,
                                                                                y_true=y_val)
        ...
        if evaluation_config['optimal_threshold'] == True:  # we use only training data to find optimal threshold
            ...
            report_val = eval_classification_report_confusion_matrix(y_pred=y_val_pred, y_true=y_val,
                                                                     title=title + ' val with optimal threshold',
                                                                     save_png=save_cm_plots, path=eval_plots_path)
            model_comparison = eval_update_model_stats(model_comparison, title + ' optimal threshold', report_val,
